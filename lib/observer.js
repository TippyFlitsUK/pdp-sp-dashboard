...
async function observerFetch(url, init, opts = {}) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      // A per-attempt timeout means the query is too slow for this timeout budget;
      // retrying with the same budget only multiplies wall-clock latency and can
      // push the response past nginx's proxy_read_timeout (surfacing as a 504).
      // Retries exist for transient HTTP 502/503/504 (Ponder restarts), not timeouts.
      if (err.name === "TimeoutError" || attempt === maxRetries) throw err
      lastErr = err
    }
...
async function observerGet(path, opts) {
  const res = await observerFetch(`${OBSERVER_URL}${path.startsWith("/") ? path : "/" + path}`, undefined, opts)
  return res.json()
}
