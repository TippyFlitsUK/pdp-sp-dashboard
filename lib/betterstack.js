...
// Query queues — separate for SP logs and dealbot (different hosts)
...
const enqueueSPLogs = createQueue(3)
const enqueueDealbot = createQueue(5)

async function queryBetterStack(sql) {
  return enqueueSPLogs(() => _queryBetterStack(sql))
}
...
async function queryDealbot(sql) {
  return enqueueDealbot(() => _queryDealbot(sql))
}
...
function validateHours(val) {
  ...
  if (isNaN(h) || h < 1 || h > 168) return 72
  return h
}
...
// Dealbot metrics tables per network
const DEALBOT_METRICS_MAINNET = "remote(t468215_infra_prod_metrics)"
const DEALBOT_METRICS_CALIBRATION = "remote(t468215_infra_staging_2_metrics)"

function getDealbotMetrics(network) {
  if (network === "calibration") return DEALBOT_METRICS_CALIBRATION
  return DEALBOT_METRICS_MAINNET
}
