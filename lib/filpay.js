...
const PDP_RAIL_OPERATORS = {
  mainnet: [
    "0x8408502033c418e1bbc97ce9ac48e5528f371a9f",
    "0x56e53c5e7f27504b810494cc3b88b2aa0645a839",
  ],
  calibration: [
    "0x02925630df557f957f70e112ba06e50965417ca0",
    "0x0c6875983b20901a7c3c86871f43fdee77946424",
  ],
}

function pdpOperatorList(network) {
  return (PDP_RAIL_OPERATORS[network] || PDP_RAIL_OPERATORS.mainnet).map(o => `'${o}'`).join(", ")
}
...
const RAIL_LATEST_CTE = `
  WITH rate_max AS (
    SELECT rail_id, MAX(block_number) AS mb
    FROM fp_rail_rate_modified
    GROUP BY rail_id
  ),
  latest_rate AS (
    SELECT DISTINCT ON (r.rail_id) r.rail_id, r.new_rate
    FROM fp_rail_rate_modified r
    JOIN rate_max m ON m.rail_id = r.rail_id AND m.mb = r.block_number
    ORDER BY r.rail_id, r.block_number DESC, split_part(r.id, '-', 2)::int DESC
  ),
  latest_lockup AS (
...
async function getAllActiveRailsByPayee(network) {
  const rows = await observerSql(network, `
    ...
    WHERE c.operator IN (${pdpOperatorList(network)})
    GROUP BY c.payee
  `)
...
async function getSpRails(network, address) {
  ...
  // Scope every CTE to this payee's rails FIRST. The shared network-wide
  // RAIL_LATEST_CTE aggregates ~915k rate-modified + ~8k settled rows before
  // filtering — the same aggregate-then-filter hot spot already removed from
  // getRailDetail. Narrowing to the SP's rail_ids up front lets Postgres use the
  // rail_id index (15s → ~1s for a large SP).
  const rows = await observerSql(network, `
    WITH my_rails AS (
      SELECT rail_id, payer, payee, operator, validator, token,
             commission_rate_bps, timestamp AS created_at
      FROM fp_rail_created
      WHERE payee = '${addr}'
        AND operator IN (${pdpOperatorList(network)})
    ),
    latest_rate AS (
      SELECT DISTINCT ON (rail_id) rail_id, new_rate
      FROM fp_rail_rate_modified
      WHERE rail_id IN (SELECT rail_id FROM my_rails)
      ORDER BY rail_id, block_number DESC
    ),
    latest_lockup AS (
      SELECT DISTINCT ON (rail_id) rail_id, new_lockup_period, new_lockup_fixed
    ...
    termination AS (
      SELECT rail_id, MAX(end_epoch) AS end_epoch FROM fp_rail_terminated
      WHERE rail_id IN (SELECT rail_id FROM my_rails) GROUP BY rail_id
    ),
    finalization AS (
      SELECT DISTINCT rail_id FROM fp_rail_finalized
      WHERE rail_id IN (SELECT rail_id FROM my_rails)
...
async function getRailDetail(network, railId) {
  ...
  const railFocusedSql = `
    ...
    FROM fp_rail_created c
    WHERE c.rail_id = ${id}
      AND c.operator IN (${pdpOperatorList(network)})
    LIMIT 1
  `
