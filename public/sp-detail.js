...
// ============================================================
// SP OVERVIEW TAB -- the central monitoring page
// ============================================================
function renderSPOverview(sp) {
  var grid = document.getElementById("sp-overview-grid")
  var pdp = sp.pdp || {}
  var fwss = sp.fwss || {}
  ...
  if (sp.hasLogs && logs.errors > 100) { healthClass = "error"; healthLabel = "Errors Detected" }
  else if (sp.hasLogs && logs.errors > 0) { healthClass = "warning"; healthLabel = "Minor Issues" }
  else if (faults > 0 && faults > proofs * 0.1) { healthClass = "warning"; healthLabel = "Proving Issues" }
  if (sp.liveness && sp.liveness.alive === false) { healthClass = "offline"; healthLabel = "Unreachable" }

  var html = ''


  // ---- Performance Section (same cards as Performance tab) ----
  html += '<div class="ov-section panel" data-section="performance">'
  html += '<div class="ov-section-header"><span class="ov-section-title performance">Performance</span><span class="ov-section-link">View details &rsaquo;</span></div>'
  html += '<div id="overview-performance-cards"><div class="loading">Loading performance data</div></div>'
  html += '</div>'

  // ---- Proving & Storage Section (same cards as Proving tab) ----
  html += '<div class="ov-section panel" data-section="proving">'
  html += '<div class="ov-section-header"><span class="ov-section-title proving">Proving & Storage</span><span class="ov-section-link">View details &rsaquo;</span></div>'
  html += '<div id="overview-proving-cards"><div class="loading">Loading proving data</div></div>'
  html += '</div>'

  // ---- Economics Section (same cards as Economics tab) ----
  html += '<div class="ov-section panel" data-section="economics">'
  html += '<div class="ov-section-header"><span class="ov-section-title economics">Economics</span><span class="ov-section-link">View details &rsaquo;</span></div>'
  html += '<div id="overview-economics-cards"><div class="loading">Loading economics data</div></div>'
  html += '</div>'

  // ---- Logs Section (same cards as Logs tab) ----
  if (sp.hasLogs) {
    html += '<div class="ov-section panel" data-section="logs">'
    html += '<div class="ov-section-header"><span class="ov-section-title logs">Logs</span><span class="ov-section-link">View details &rsaquo;</span></div>'
    html += summaryGrid([
      { label: "Errors", value: formatNum(logs.errors || 0), cls: "errors" },
      { label: "Warnings", value: formatNum(logs.warns || 0), cls: (logs.warns || 0) > 0 ? "warnings" : "zero" },
  ...
  // Wire up section clicks
  grid.querySelectorAll(".ov-section").forEach(function(el) {
    el.querySelector(".ov-section-header").addEventListener("click", function() {
      switchTab(el.dataset.section)
    })
  })
}

function keyMetric(label, value, sub, section, cls) {
  return '<div class="key-metric" data-section="' + section + '">' +
    '<div class="km-label">' + label + '</div>' +
    '<div class="km-value ' + (cls || '') + '">' + value + '</div>' +
    (sub ? '<div class="km-sub">' + sub + '</div>' : '') +
  '</div>'
}

function osStat(label, value, cls) {
...
// Update proving overview with same cards as Proving tab
function updateOverviewProving(sp) {
  var el = document.getElementById("overview-proving-cards")
  if (!el || !spDataCache.proving) return
  var prov = spDataCache.proving.provider || {}
  ...
  el.innerHTML = summaryGrid([
    ...
    { label: "Faulted Periods (All Time)", value: String(totalFaultPeriods), cls: totalFaultPeriods > 0 ? "errors" : "" },
    { label: "Faulted Periods (7d)", value: String(faultPeriods7d), cls: faultPeriods7d > 0 ? "errors" : "green" },
    { label: "Last Success (%)", value: lastSuccess, cls: "green" },
  ])
}

// Update performance stats when data loads
function updateOverviewPerformance() {
  if (!spDataCache.performance) return
  var counters = spDataCache.performance.counters || []
  var byCheck = {}
  for (var i = 0; i < counters.length; i++) {
    var c = counters[i]
    if (!byCheck[c.checkType]) byCheck[c.checkType] = { success: 0, failed: 0 }
  ...
  var ds = byCheck["dataStorage"] || { success: 0, failed: 0 }
  var rt = byCheck["retrieval"] || { success: 0, failed: 0 }
  var dsTotal = ds.success + ds.failed
  var dsPct = dsTotal > 0 ? ((ds.success / dsTotal) * 100).toFixed(1) : "N/A"
  var dsCls = dsPct !== "N/A" ? rateClass(dsPct) : ""
  var rtTotal = rt.success + rt.failed
  var rtPct = rtTotal > 0 ? ((rt.success / rtTotal) * 100).toFixed(1) : "N/A"
  var rtCls = rtPct !== "N/A" ? rateClass(rtPct) : ""

  // Update overview with same cards as Performance tab
  var el = document.getElementById("overview-performance-cards")
  if (el) {
    var byCheck = {}
    for (var j = 0; j < counters.length; j++) {
      var c2 = counters[j]
      if (!byCheck[c2.checkType]) byCheck[c2.checkType] = { success: 0, failed: 0 }
      if (c2.value === "success") byCheck[c2.checkType].success += c2.cnt
      else if (c2.value && c2.value.startsWith("failure")) byCheck[c2.checkType].failed += c2.cnt
    }
...
// Update economics stats when data loads
function updateOverviewEconomics() {
  var el = document.getElementById("overview-economics-cards")
  if (!el || !spDataCache.economics) return
  var s = spDataCache.economics.summary || {}
  var acct = spDataCache.economics.account || {}
  var settled = Number(acct.fundsCollected || 0) / 1e18

  // Calculate expected settlement
  var currentEpoch = Math.floor((Date.now() / 1000 - 1598306400) / 30)
  var expected = 0
  var rails = spDataCache.economics.rails || []
  for (var i = 0; i < rails.length; i++) {
    var r = rails[i]
    if (r.state === "ACTIVE" && r.settledUpto && r.paymentRate !== "0") {
      var diff = currentEpoch - Number(r.settledUpto)
      if (diff > 0) expected += diff * Number(r.paymentRate) / 1e18
    }
  }
  var totalRevenue = settled + expected

  el.innerHTML = summaryGrid([
    { label: "Active Rails", value: formatNum(s.activeRails) },
    { label: "Total Rails", value: formatNum(s.totalRails) },
    { label: "Expected Settlement", value: "$" + expected.toFixed(2), cls: "green" },
  ]) + summaryGrid([
    { label: "Total Settled", value: "$" + settled.toFixed(2) },
